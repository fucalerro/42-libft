...
const char *strstr_tester(void)
{
	...
	char *s3 = "rth";
	char *s4 = "a";
	char *s5 = "ha123ystack412haystack";
	char *ss[5] = {s1, s2, s3, s4, s5};

	for (int i = 0; i < 5; i++)
		for (int j = 0; j < 5; j++)
		{
			char *ft_result = ft_strstr(ss[i], ss[j]);
			char *std_result = strstr(ss[i], ss[j]);
...
const char *strnstr_tester(void)
{
	...
	for (int i = 0; i < 5; i++)
		for (int j = 0; j < 5; j++)
			// for (int n = -2; n < 25; n++)
			{
				char *ft_result = ft_strnstr(ss[i], ss[j], n);
				char *std_result = strnstr(ss[i], ss[j], n);
...
{
	char *str[] = {"--1230", "  +1230", "-1230", "   -1230", "  -  1230", "asdd", ""};
...
const char	*bzero_tester(void)
{
	...
	size_t int_n = sizeof(intarr1) / sizeof(intarr1[0]);
	size_t char_n = strlen(charr1);
...
const char *memmove_tester(void)
{
    ...
    for (int c = 0; c < 127; c++)
	{
        ...
        if (memcmp(intarr1, intarr2, int_n - 1) != 0 || strcmp(charr1, charr2) != 0)
            return ERR;
    }
    return OK;
}
...
const char *strncpy_tester(void) {
    ...
    const char *src = "Hello, World!";
    size_t max_len = sizeof(dest) - 1; // Maximum length to avoid buffer overflow
...
const char *strjoin_tester(void)
{
	...
	for (size_t i = 0; i < 2; i++)
		for (size_t j = 0; j < 2; j++)
			...
			{
				printf("%s\n%s\n%s\n%s\n", s1[i], s1[j], res[i + j], ft_strjoin(s1[i], s1[j]));
				return ERR;
			}
	return OK;
}
